#ifndef SOFT_SERVO_H_
#define SOFT_SERVO_H_

#include <avr/io.h>
#include <stdint.h>

#define MAX_SOFT_SERVOS 6

// 1. Enciende el metrónomo global (Timer1)
void SoftServo_Init(void);

// 2. Conecta un servo y le configura sus PROPIOS límites físicos y analógicos
// Formato: Canal, Puerto, DDR, Pin, minTick, maxTick, minADC, maxADC
void SoftServo_Attach(uint8_t canal, volatile uint8_t *puerto, volatile uint8_t *ddr, uint8_t pin, uint16_t min_tick, uint16_t max_tick, uint16_t min_adc, uint16_t max_adc);

// 3. Mueve el servo
void SoftServo_SetFromADC(uint8_t canal, uint16_t adc_val);

#endif /* SOFT_SERVO_H_ */